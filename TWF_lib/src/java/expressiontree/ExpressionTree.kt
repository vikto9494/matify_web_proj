...
class ExpressionTreeParser(
    ...
    private fun normalisePositionInParseError(parserError: ParserError): ParserError {
        ...
        else parserError.endPosition + shift * newLineMspace.length
        root.nodeType = NodeType.ERROR
        root.startPosition = parserError.position + shift * newLineMspace.length
        root.endPosition = endPosition
        root.value = parserError.description
        return ParserError(parserError.position + shift * newLineMspace.length, parserError.description, endPosition)
    }
        ...
        while (nodeIndex < oldTreeActualParent.children.size) {
            ...
            if (node.type == MathMlTagTreeNode.Type.EXPRESSION_PART_STRING) {
                ...
                while (currentPosition < node.value.length) {
                    ...
                    if (completeSign != null) {
                    ...
                    } else if (isBinarySignPart(node.value[currentPosition], isMathML)) {
                        ...
                        while (currentPosition < node.value.length) {
                            ...
                            if (isBinarySignPart(node.value[currentPosition], isMathML) && getTexFunction(node.value, currentPosition) == null) {
                                value += node.value[currentPosition]
                                currentPosition++
                            } else {
